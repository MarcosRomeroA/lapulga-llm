...
Runs the training loop locally using the PyTorch backend with custom BPE tokenizer.
"""
import torch
...
def main() -> None:
    ...
    tokens = tokenize_stream(train_stream, tokenizer, target_tokens=train_config.train_tokens)
    print(f"Train tokens: {len(tokens):,}")

    val_stream = fetch_tinystories_stream(split="validation")
    val_tokens = tokenize_stream(val_stream, tokenizer, target_tokens=train_config.val_tokens)
    print(f"Validation tokens: {len(val_tokens):,}")

    # 5. Training
    execute_training(model=model, tokens=tokens, train_config=train_config)
    ...
    output_text: str = generate_text(model, tokenizer, test_prompt, max_tokens=60, temperature=0.7)
